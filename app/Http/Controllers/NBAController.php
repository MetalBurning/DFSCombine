...
class NBAController extends Controller
{
    ...
    public function loadHistory(Request $request)
    {
        ...
        $savedJSON = DB::table('UserSaveData')->select('created_at', 'id', 'title', 'site')->where([['userID', '=', Auth::user()->id],['sport' ,'=', 'NBA']])->whereNull('deleted_at')->orderBy('created_at', 'desc')->skip($request->input('endIndex'))->take(10)->get();
    ...
    public function downloadDrafts(Request $request)
    {
        ...
        DB::table('UserActions')->insert(
            ['userID' => Auth::user()->id, 'sport' => 'NBA', 'action' => 'downloadDrafts', 'actionDetails' => $request->input('downloadDrafts')]
        );
    }
    public function buildDraft(Request $request)
    {
        ...
        DB::table('UserActions')->insert(
            ['userID' => Auth::user()->id, 'sport' => 'NBA', 'action' => 'build', 'actionDetails' =>  $request->input('builtDrafts')]
        );
    }
    ...
    public function create(Request $request)
    {
        if($request->input() != NULL) {
          ...
          $id = DB::table('UserSaveData')->insertGetId(
              ['userID' => Auth::user()->id, 'userSaveJSON' => $postObject, 'title' => $title, 'sport' => 'NBA', 'site' => $site]
          );
          $savedJSON = DB::table('UserSaveData')->select('userSaveJSON', 'title', 'id', 'site')->where([['userID', '=', Auth::user()->id],['id', '=', $id],['sport' ,'=', 'NBA']])->whereNull('deleted_at')->first();
          return Response::json($savedJSON, 200);
        }
    }
