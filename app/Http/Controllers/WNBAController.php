...
class WNBAController extends Controller
{
    ...
    public function loadFanDuelPlayers(Request $request)
    {
        ...
        DB::table('UserActions')->insert(
            ['userID' => Auth::user()->id, 'sport' => 'WNBA', 'action' => 'playerLoad', 'actionDetails' =>  'FanDuel', 'actionDetailsLarge' => json_encode($players)]
        );
    ...
    public function loadDraftKingsPlayers(Request $request)
    {
        ...
        DB::table('UserActions')->insert(
            ['userID' => Auth::user()->id, 'sport' => 'WNBA', 'action' => 'playerLoad', 'actionDetails' => 'DraftKings', 'actionDetailsLarge' => json_encode($players)]
        );
        return Response::json($players, 200);
    }
    public function loadDraftKingsFPPG(Request $request)
    {
      ...
      //  try {
        ...
        // }
        DB::table('UserActions')->insert(
            ['userID' => Auth::user()->id, 'sport' => 'WNBA', 'action' => 'playerFPPGLoad', 'actionDetails' => 'DraftKings', 'actionDetailsLarge' => json_encode($players)]
        );
        return Response::json($players, 200);
    }
    public function loadHistory(Request $request)
    {
        ...
        $savedJSON = DB::table('UserSaveData')->select('created_at', 'id', 'title', 'site')->where([['userID', '=', Auth::user()->id],['sport' ,'=', 'WNBA']])->whereNull('deleted_at')->orderBy('created_at', 'desc')->skip($request->input('endIndex'))->take(10)->get();
    ...
    public function downloadDrafts(Request $request)
    {
        ...
        DB::table('UserActions')->insert(
            ['userID' => Auth::user()->id, 'sport' => 'WNBA', 'action' => 'downloadDrafts', 'actionDetails' => $request->input('downloadDrafts')]
        );
    }
    public function buildDraft(Request $request)
    {
        ...
        DB::table('UserActions')->insert(
            ['userID' => Auth::user()->id, 'sport' => 'WNBA', 'action' => 'build', 'actionDetails' =>  $request->input('builtDrafts')]
        );
    }
    ...
    public function create(Request $request)
    {
        if($request->input() != NULL) {
          ...
          $id = DB::table('UserSaveData')->insertGetId(
              ['userID' => Auth::user()->id, 'userSaveJSON' => $postObject, 'title' => $title, 'sport' => 'WNBA', 'site' => $site]
          );
          $savedJSON = DB::table('UserSaveData')->select('userSaveJSON', 'title', 'id', 'site')->where([['userID', '=', Auth::user()->id],['id', '=', $id],['sport' ,'=', 'WNBA']])->whereNull('deleted_at')->first();
          return Response::json($savedJSON, 200);
        }
    }
    public function read(Request $request)
    {
        ...
        $savedJSON = DB::table('UserSaveData')->select('userSaveJSON', 'title', 'id', 'site')->where([['userID', '=', Auth::user()->id],['id', '=', $request->input('id')],['sport' ,'=', 'WNBA']])->whereNull('deleted_at')->first();
...
$savedJSON = '';
        // DB::table('UserSaveData')->where([ ['userID', '=', Auth::user()->id], ['sport' ,'=', 'WNBA'], ['id', '=', $request->input('id')] ])->whereNull('deleted_at')->update(['title' => $request->input('title')]);
        // DB::table('UserSaveData')->where([ ['userID', '=', Auth::user()->id], ['sport' ,'=', 'WNBA'], ['id', '=', $request->input('id')] ])->whereNull('deleted_at')->update(['userSaveJSON' => $request->input('postObject') ]);
        // $savedJSON = DB::table('UserSaveData')->select('userSaveJSON', 'title', 'id', 'site')->where([['userID', '=', Auth::user()->id],['id', '=',  $request->input('id')],['sport' ,'=', 'WNBA']])->whereNull('deleted_at')->first();
        return Response::json($savedJSON, 200);
    }
    public function updateTitle(Request $request)
    {
        ...
        $currentTime = Carbon::now();
$savedJSON = '';
        //$savedJSON = DB::table('UserSaveData')->where([ ['userID', '=', Auth::user()->id], ['sport' ,'=', 'WNBA'], ['id', '=', $request->input('id')] ])->whereNull('deleted_at')->update(['title' => $request->input('title')]);
        return Response::json($savedJSON, 200);
    }
    public function delete(Request $request)
    {
        ...
        $currentTime = Carbon::now();
$savedJSON = '';
        //$savedJSON = DB::table('UserSaveData')->where([ ['userID', '=', Auth::user()->id], ['sport' ,'=', 'WNBA'], ['id', '=', $request->input('id')] ])->whereNull('deleted_at')->update(['deleted_at' => $currentTime->toDateTimeString()]);
        return Response::json($savedJSON, 200);
    }
