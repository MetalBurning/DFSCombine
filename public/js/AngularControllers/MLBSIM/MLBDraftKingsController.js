angular.module('MLBApp').controller('MLBController', ['$http', '$scope', '$filter', '$uibModal', '$window', function ($http, $scope, $filter, $uibModal, $window) {
    ...
    $scope.SelectedPosition = 'P1';     // set the default search/filter term
    ...
    $scope.loadDKPlayers = function (event) {
      ...
      $http.post("/MLB/loadDraftKingsPlayers", formData, {
          headers: {
              "Content-Type": undefined,
              transformRequest: angular.identity
          }
    ...
    $scope.selectTopActualPlayers = function() {
      ...
      for(var j = 0; j < 5; j++) {
        if(j < 3) {
          $scope.addPlayerToPool(allPs[j], 'P1');
        ...
        if(j > 0) {
          $scope.addPlayerToPool(allPs[j], 'P2');
          $scope.addPlayerToPool(allOFs[j], 'OF2');
          $scope.addPlayerToPool(allOFs[j], 'OF3');
    ...
    $scope.DownloadDraftCSV = function () {
        ...
        csvContent = csvContent + "P,P,C,1B,2B,3B,SS,OF,OF,OF\n";
    ...
    $scope.removePlayerFromPool = function (player, position)
    {
      if($scope.playerInPool(player, position))
      {
        ...
        {
            case 'P1':
                $scope._P1PlayerPool.splice($scope._P1PlayerPool.indexOf(player), 1);
                break;
            case 'P2':
                $scope._P2PlayerPool.splice($scope._P2PlayerPool.indexOf(player), 1);
                break;
            case 'C':
    ...
    $scope.addPlayerToPool = function (player, position)
    {
      if($scope.SelectedPosition === '') {
        ...
        if (!$scope.playerInPool(player, position))
        {
            ...
            {
                case 'P1':
                    $scope._P1PlayerPool.push(player);
                    break;
                case 'P2':
                    $scope._P2PlayerPool.push(player);
                    break;
                case 'C':
    ...
    $scope.playerInPool = function (player, position)
    {
        ...
        {
            case 'P1':
                if ($scope._P1PlayerPool.indexOf(player) > -1)
                {
                    return true;
                }
                break;
            case 'P2':
                if ($scope._P2PlayerPool.indexOf(player) > -1)
                {
                    return true;
                }
                break;
    ...
    $scope.buildDrafts = function () {
        ...
        if ($scope._P1PlayerPool.length == 0 ||
            $scope._P2PlayerPool.length == 0 ||
        ...
        $scope.totalPossibleDraftsToBeCreated = $scope._P1PlayerPool.length * $scope._P2PlayerPool.length *
        ...
        $scope._P1PlayerPool.forEach(function(P1Player) {
            var tempDraft = {};
            tempDraft['P1'] = P1Player;
            $scope._P2PlayerPool.forEach(function(P2Player) {
              tempDraft['P2'] = P2Player;
                        ...
                        $scope._OF1PlayerPool.forEach(function(OF1Player) {
                          ...
                          $scope._OF2PlayerPool.forEach(function(OF2Player) {
                            ...
                            $scope._OF3PlayerPool.forEach(function(OF3Player) {
                              ...
                              var finalPlayerList = [];
                              finalPlayerList.push(tempDraft['P1']);
                              finalPlayerList.push(tempDraft['P2']);
                              ...
                              var tempPlayerNames = [];
                              tempPlayerNames.push(tempDraft['P1']._Name);
                              tempPlayerNames.push(tempDraft['P2']._Name);
                                 ...
                                 if(mlb.removeDups)
                                 {
                                   ...
                                   if(tempPlayerNamesList.length > 0) {
                                     for(var j = tempPlayerNamesList.length-1; j >= 0; j--) {
                                       ...
                                       if(tempPlayerNames.indexOf(tempPlayerNamesList[j][9]) !== -1) {
                                         playersInDraft++;
                                       }
                                       if(playersInDraft === 10) {
    ...
    $scope.isDraftTeamValid = function (draft) {
        ...
        draft.forEach(function (player) {
          if(player._Position.indexOf('P') === -1) {
            if(!teamHitters.hasOwnProperty(player._Team)) {
              teamHitters[player._Team] = 1;
            } else {
              teamHitters[player._Team]++;
            }
          }
        });
        for (var teamName in teamHitters) {
          if(teamHitters[teamName] > 5) {
            return false;
          }
    ...
    $scope.openSaveDialog = function () {
        ...
        var postObject = {
            ...
            _P1PlayerPool : $scope._P1PlayerPool,
            _P2PlayerPool : $scope._P2PlayerPool,
    ...
    $scope.loadPlayersFromSave = function(savedData) {
      ...
      $scope._AllPlayers.forEach(function(singlePlayer) {
        ...
        $scope.loadPlayerInPool(savedData._P1PlayerPool, singlePlayer, 'P1');
        $scope.loadPlayerInPool(savedData._P2PlayerPool, singlePlayer, 'P2');
