...
angular.module('NFLApp').controller('NFLController', ['$http', '$scope', '$filter', '$uibModal', '$window', function ($http, $scope, $filter, $uibModal, $window) {
    ...
    $scope.alerts = [
      { type: 'info', msg: 'Currently untested. Will be available when 2017 season starts. Please Upload/Load Players...', number: 1 }
    ];
    ...
    $scope.setPlayerRanking = function() {
      ...
      var allQBs = $filter('positionDK')(injuredPlayers, 'QB');
      var allRBs = $filter('positionDK')(injuredPlayers, 'RB');
      var allWRs = $filter('positionDK')(injuredPlayers, 'WR');
      var allTEs = $filter('positionDK')(injuredPlayers, 'TE');
      var allFLEXs = $filter('positionDK')(injuredPlayers, 'FLEX');
      var allDSTs = $filter('positionDK')(injuredPlayers, 'DST');

      var outPlayers =  $filter('removeOut')(orderedFPPGPlayers);
      var allInjQBs = $filter('positionDK')(outPlayers, 'QB');
      var allInjRBs = $filter('positionDK')(outPlayers, 'RB');
      var allInjWRs = $filter('positionDK')(outPlayers, 'WR');
      var allInjTEs = $filter('positionDK')(outPlayers, 'TE');
      var allInjFLEXs = $filter('positionDK')(outPlayers, 'FLEX');
      var allInjDSTs = $filter('positionDK')(outPlayers, 'DST');
        ...
        if(!player._playerInjured) {
          ...
          switch(player._Position) {
            ...
            case 'TE':
              ...
              break;
            case 'FLEX':
              playerRank = allInjFLEXs.indexOf(player) + 1;
              break;
            case 'DST':
                  ...
                  $scope._WR2PlayerPool.forEach(function(WR2Player) {
                    ...
                    $scope._WR3PlayerPool.forEach(function(WR3Player) {
                      ...
                      $scope._TEPlayerPool.forEach(function(TEPlayer) {
                        ...
                        $scope._FLEXPlayerPool.forEach(function(FLEXPlayer) {
    ...
    $scope.selectTopActualPlayers = function() {
      ...
      var allQBs = $filter('positionDK')(NonInjuredPlayers, 'QB');
      var allRBs = $filter('positionDK')(NonInjuredPlayers, 'RB');
      var allWRs = $filter('positionDK')(NonInjuredPlayers, 'WR');
      var allTEs = $filter('positionDK')(NonInjuredPlayers, 'TE');
      var allFLEXs = $filter('positionDK')(NonInjuredPlayers, 'FLEX');
      var allDSTs = $filter('positionDK')(NonInjuredPlayers, 'DST');
