angular.module('NHLApp').controller('NHLController', ['$http', '$scope', '$filter', '$uibModal', '$window', function ($http, $scope, $filter, $uibModal, $window) {
    ...
    $scope.loadPlayers = function (event) {
      ...
      $http.post("/NHL/loadDraftKingsPlayers", formData, {
          headers: {
    ...
    $scope.selectTopActualPlayers = function() {
      ...
      var allWs = $filter('positionDK')(orderedPlayers, 'W');
      var allDs = $filter('positionDK')(orderedPlayers, 'D');
    ...
    $scope.selectTopFPPGPlayers = function() {
      ...
      var allWs = $filter('positionDK')(NonInjuredPlayers, 'W');
      var allDs = $filter('positionDK')(NonInjuredPlayers, 'D');
      ...
      for(var j = 0; j < allUTILs.length; j++) {
        if( j < 6 ) {
          $scope.addPlayerToPool(allWs[j], 'UTIL');
        }
      }
    ...
    $scope.DownloadDraftCSV = function () {
        ...
        csvContent = csvContent + "C,C,W,W,W,D,D,G,UTIL\n";
    ...
    $scope.setPlayerRanking = function() {
      ...
      var allWs = $filter('positionDK')(injuredPlayers, 'W');
      var allDs = $filter('positionDK')(injuredPlayers, 'D');
      var allGs = $filter('positionDK')(injuredPlayers, 'G');
      $scope._AllPlayers.forEach(function(player) {
        var playerRank = [];
        ...
        if(player._Position.indexOf('W') !== -1) {
          playerRank.push(allWs.indexOf(player) + 1);
        }
        if(player._Position.indexOf('D') !== -1) {
          playerRank.push(allDs.indexOf(player) + 1);
        }
        if(player._Position.indexOf('G') !== -1) {
          playerRank.push(allGs.indexOf(player) + 1);
        }
    ...
    $scope.buildDrafts = function () {
        ...
        if ($scope._C1PlayerPool.length == 0 ||
            ...
            $scope._UTILPlayerPool.length == 0 ||
        ...
        $scope._W3PlayerPool.length * $scope._UTILPlayerPool.length *
                      ...
                      $scope._D1PlayerPool.forEach(function(D1Player) {
                        ...
                        $scope._D2PlayerPool.forEach(function(D2Player) {
                          ...
                          $scope._GPlayerPool.forEach(function(GPlayer) {
                            ...
                            var tempPlayerNames = [];
                            tempPlayerNames.push(tempDraft['C1']._Name);
                            tempPlayerNames.push(tempDraft['C2']._Name);
                            tempPlayerNames.push(tempDraft['W1']._Name);
                            tempPlayerNames.push(tempDraft['W2']._Name);
                            tempPlayerNames.push(tempDraft['W3']._Name);
                            tempPlayerNames.push(tempDraft['D1']._Name);
                            tempPlayerNames.push(tempDraft['D2']._Name);
